...
function appendDoc(panel, elem, panel_class) {
	if (typeof panel_class === 'undefined') panel_class = 'panel-footer'
	var doc = elem.querySelector('.doc')
	if (doc && doc.parentNode == elem) {
		panel.appendChild(elemWithAttr('div', {'class': panel_class}, doc))
	}
}
...
function wrapAnchor(anchor) {
	...
	panel_heading.appendChild(h3)
	panel.appendChild(panel_heading)
	appendDoc(panel, anchor, 'panel-body')
	annotate(anchor)
...
function annotateForm(form, suggestion) {
	...
	for (i=0; i<inputs.length; i++) {
		var input = inputs[i]
		// TODO: We need to not stomp over existing class here
		input.setAttribute('class', 'form-control')
		form.appendChild(
			elemWithAttr('div', {'class': 'form-group'},
				elemWithAttr('label', {'for': input.name}, input.name), 
				input
			)
		)
	}
	form.appendChild(elemWithAttr('button', {'class': 'btn', 'type': 'submit'}, button_text))
	return form
}
